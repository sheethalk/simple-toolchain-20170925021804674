;(function(angular){
  ...
  angular.module('pickadate', [])
    ...
    .provider('pickadateI18n', function() {
      var defaults = {
        'prev': '<i class="fa fa-caret-left fa-2x"></i>',
      ...
      return function(format, options) {
        ...
        return {
          ...
          setRestrictions: function(restrictions) {
            minDate       = this.parseDate(restrictions.minDate) || new Date() - 1;
        ...
        '<div class="pickadate" ng-show="displayPicker" ng-style="modalStyles">' +
          ...
          '<div class="pickadate-body">' +
            '<div class="pickadate-main">' +
              '<ul class="pickadate-cell day-header">' +
                '<li class="day-header" ng-repeat="dayName in dayNames">' +
                  '{{dayName}}' +
                '</li>' +
              '</ul>' +
              '<ul class="pickadate-cell">' +
                '<li ng-repeat="dateObj in dates" class="cal-days" ng-click="setDate(dateObj)" ng-class="classesFor(dateObj)">' +
                  '{{dateObj.date | date:"d"}}' +
                '</li>' +
              '</ul>' +
            '</div>' +
