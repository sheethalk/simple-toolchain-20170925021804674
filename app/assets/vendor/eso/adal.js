...
AuthenticationContext.prototype.logOut = function () {
    ...
    var urlNavigate = this.config.authorizationUrl + 'endsession?' + logout;
...
AuthenticationContext.prototype.getStartPage = function () {
    return this._getItem(this.CONSTANTS.STORAGE.START_PAGE);
};
...
AuthenticationContext.prototype._createUser = function (idToken) {
    ...
    if (parsedJson && parsedJson.hasOwnProperty('aud')) {
        if (parsedJson.aud === this.config.clientId) {
            user = {
                userName: '',
                profile: parsedJson
            };

            if (parsedJson.hasOwnProperty('sub')) {
                user.userName = parsedJson.sub;
            } else if (parsedJson.hasOwnProperty('upn')) {
...
AuthenticationContext.prototype._getNavigateUrl = function (responseType, resource) {
    ...
    var urlNavigate = this.config.authorizationUrl + 'authorize' + this._serialize(responseType, this.config, resource);// + this._addClientId();
...
AuthenticationContext.prototype._serialize = function (responseType, obj, resource) {
    var str = [];
    if (obj !== null) {
        str.push('?response_type=' + encodeURIComponent(obj.response_type));
